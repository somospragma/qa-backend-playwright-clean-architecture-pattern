...
import { BookingCreateService } from "@booking/application";
import { IBooking, IBookingCreateService, IBookingDates } from "@booking/ports";
...
test.describe("List API Test Suite", () => {
    ...
    test.beforeAll(async () => {
        ...
        bookingCreateService = new BookingCreateService(apiContext);
    });

    testDataBooking.forEach((booking, index) => {
        test(`Validar el create con el Token - Caso ${index + 1}`, async () => {
            ...
            await test.step("Validar la respuesta del servicio", async () => {
                ...
                expect(bookingCreateService.responsePlaywright.status()).toBe(201);
            });
        });
    });
