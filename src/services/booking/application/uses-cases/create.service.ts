...
import { IBooking, IBookingResponse, IBookingCreateService, IValidatorJsonSchemaHelper, IValidateBookingEntity } from "@booking/ports";
...
export class BookingCreateService implements IBookingCreateService {
    ...
    async consumeService(token: string, booking: IBooking): Promise<void> {
        ...
        this.loggerHelper.logRequest("POST", `${url}${path}`, headers, booking);
        if (url) {
            try {
